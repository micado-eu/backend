...
export class ProcessController {
  ...
  async translatedunion (
    ...
    return this.processRepository.dataSource.execute("select *, (select to_jsonb(array_agg(pt.id_topic)) from process_topic pt where pt.id_process=t.id) as topics, (select to_jsonb(array_agg(pu.id_user_types)) from process_users pu where pu.id_process=t.id) as users from process t inner join process_translation tt on t.id=tt.id and tt.lang='" +
      currentlang + "' union select *, (select to_jsonb(array_agg(pt.id_topic)) from process_topic pt where pt.id_process=t.id) as topics, (select to_jsonb(array_agg(pu.id_user_types)) from process_users pu where pu.id_process=t.id) as users from process t inner join process_translation tt on t.id = tt.id and tt.lang = '" +
      defaultlang +
