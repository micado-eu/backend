...
@model({
  settings: {
    idInjection: false,
    postgresql: { schema: 'micadoapp', table: 'information_translation' }
  }
})
export class InformationTranslation extends Entity {
  @property({
    type: 'number',
    required: true,
    scale: 0,
  ...
  constructor(data?: Partial<InformationTranslation>) {
    super(data);
  }
}

export interface InformationTranslationRelations {
  // describe navigational properties here
}

export type InformationTranslationWithRelations = InformationTranslation & InformationTranslationRelations;
