...
@model({
  settings: {
    idInjection: false,
    postgresql: {schema: 'micadoapp', table: 'event_category_translation_prod'}
  }
})
export class EventCategoryTranslationProd extends Entity {
  @property({
    type: 'number',
    required: true,
    scale: 0,
  ...
  constructor(data?: Partial<EventCategoryTranslationProd>) {
    super(data);
  }
}

export interface EventCategoryTranslationProdRelations {
  // describe navigational properties here
}

export type EventCategoryTranslationProdWithRelations = EventCategoryTranslationProd & EventCategoryTranslationProdRelations;
