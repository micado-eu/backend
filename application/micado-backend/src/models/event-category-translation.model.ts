...
@model({
  settings: {
    idInjection: false,
    postgresql: { schema: 'micadoapp', table: 'event_category_translation' }
  }
})
export class EventCategoryTranslation extends Entity {
  @property({
    type: 'number',
    required: true,
    scale: 0,
  ...
  constructor(data?: Partial<EventCategoryTranslation>) {
    super(data);
  }
}

export interface EventCategoryTranslationRelations {
  // describe navigational properties here
}

export type EventCategoryTranslationWithRelations = EventCategoryTranslation & EventCategoryTranslationRelations;
