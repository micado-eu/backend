...
@model({
  settings: {
    idInjection: false,
    postgresql: {schema: 'micadoapp', table: 'event_translation_prod'}
  }
})
export class EventTranslationProd extends Entity {
  @property({
    type: 'number',
    required: true,
    scale: 0,
  ...
  constructor(data?: Partial<EventTranslationProd>) {
    super(data);
  }
}

export interface EventTranslationProdRelations {
  // describe navigational properties here
}

export type EventTranslationProdWithRelations = EventTranslationProd & EventTranslationProdRelations;
