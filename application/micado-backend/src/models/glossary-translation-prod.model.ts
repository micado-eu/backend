...
@model({
  settings: {
    idInjection: false,
    postgresql: {schema: 'micadoapp', table: 'glossary_translation_prod'}
  }
})
export class GlossaryTranslationProd extends Entity {
  @property({
    type: 'number',
    required: true,
    scale: 0,
  ...
  constructor(data?: Partial<GlossaryTranslationProd>) {
    super(data);
  }
}

export interface GlossaryTranslationProdRelations {
  // describe navigational properties here
}

export type GlossaryTranslationProdWithRelations = GlossaryTranslationProd & GlossaryTranslationProdRelations;
